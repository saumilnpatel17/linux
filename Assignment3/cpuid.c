...
atomic_t cmpe283_assign3_atm_individual_exit_count_array[69] = {0};
EXPORT_SYMBOL_GPL(cmpe283_assign2_atm_exit_count);
EXPORT_SYMBOL_GPL(cmpe283_assign2_atm_time_count);
EXPORT_SYMBOL_GPL(cmpe283_assign3_atm_individual_exit_count_array);
...
int kvm_emulate_cpuid(struct kvm_vcpu *vcpu)
{
	...
	if(eax == 0x4fffffff) {
		...
		if( (ecx < 0) || (ecx > 68) || (ecx == 35) || (ecx == 38) || (ecx == 42) || (ecx == 65) ){
			...
			eax = atomic_read(&cmpe283_assign3_atm_individual_exit_count_array[ecx]);
			if( (eax == 0xffffffff) ){
				eax = 0;
			}
			ebx = 0;
